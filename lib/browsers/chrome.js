...
chrome.spawn = function (options) {
  return new Promise((resolve, reject) => {
    ...
    this.chromeChild = spawn(
      location,
      this.options.chromeFlags || [
        '--headless',
        '--disable-gpu',
      ...
      CDP.Version()
        .then((info) => {
          ...
          this.webSocketDebuggerURL =
            info.webSocketDebuggerUrl || 'ws://localhost:9222/devtools/browser';
    ...
    Promise.all([
      ...
      .then(() => {
        ...
        Network.requestWillBeSent((params) => {
          ...
          if (!tab.prerender.initialFrameId) {
            tab.prerender.initialFrameId = params.frameId;
          }
          ...
          // stop redirects from javascript
          if (
            params.type === 'Document' &&
            tab.prerender.initialFrameId === params.frameId &&
            getOrigin(tab.prerender.url) !== getOrigin(params.request.url) &&
            !tab.prerender.followRedirects &&
            !this.options.followRedirects
          ) {
            tab.prerender.receivedRedirect = true;
            tab.prerender.statusCode = 307; // moved temporarily
            tab.prerender.headers = {
              Location: params.request.url,
            };
            tab.prerender.content = 'Moved';
        ...
        Network.responseReceived((params) => {
          ...
          if (tab.prerender.requests[params.requestId]) {
            ...
            if (
              ...
              if (tab.prerender.statusCode == 304)
                tab.prerender.statusCode = 200;
            }
          }
        });
...
chrome.checkIfPageIsDoneLoading = function (tab) {
  return new Promise((resolve, reject) => {
    if (
      !(tab.prerender.domContentEventFired || tab.prerender.receivedRedirect)
    ) {
